...
# def AutoCorrGPU(x, corrLength = "same"):
#     # x : spike Times in ms
#     N = len(x)
#     nPointFFT = int(np.power(2, np.ceil(np.log2(len(x)))))
#     x = np.concatenate((x, np.zeros((nPointFFT - N, ))))
#     out = MyAutoCorrFunc(x, nPointFFT);
#     return out
...
#    [foldername, alpha, tau_syn, bidirType, tag, NE, NI, xi, nTheta, contrast,  simDuration, nTrials, dt] = DefaultArgs(sys.argv[1:], ['', '', 3.0, '',  '', 20000, 20000, '0.8', 8, 100.0,  100000, 1, 0.05])
    [alpha, tau_syn, bidirType, K, NE, NI, tag, xi, nTheta, contrast,  simDuration, nTrials, dt] = DefaultArgs(sys.argv[1:], ['', 3.0, '',  '', 20000, 20000, '', '0.8', 8, 100.0, 25000, 1, 0.05])
    foldername = ''
...
#    simDuration = spkTimeEnd - spkTimeStart
    ...
    filebase = '/homecentral/srao/Documents/code/cuda/cudanw/data/bidir/i2i/p8/'        
    if tag == '':
        if NI == 20000:
            tag = 'NI2E4'
        elif NI == 50000:
    ...
    if tau_syn == 3:
        if bidirType == 'e2e':
            ...
            if K == 500:
                foldername = 'i2i/p%s/'%(alpha)
            elif K == 50 and NI == 30000 and NE == 30000:
                print NE, NI                
            elif K == 2000:
                foldername = 'i2i/p%s/K2000/'%(alpha)
            tag = tag + 'I2I'            
    else:
        if bidirType == 'e2e':
            ...
            if K == 500 and NI == 20000:
                foldername = 'i2i/tau%s/p%s/'%(tau_syn, alpha)
            elif K == 2000:
                foldername = 'i2i/tau%s/K2000/p%s/'%(tau_syn, alpha)
            elif K == 1000:
                if tau_syn == 3:
                    foldername = 'i2i/p%s/'%(alpha)
                else:
                    foldername = 'i2i/tau%s/p%s/'%( tau_syn, alpha)                
                tag = tag + 'I2I'
            if K == 500 and NI == 30000 and NE == 30000:
                print NE, NI
    if tau_syn ==3:
        if NI == 20000 and NE == 20000 and int(alpha) == 0:
            filebase = ''
            foldername = '/homecentral/srao/cuda/data/pub/bidir/p0/'
    if NI == 30000 and NE == 30000:
        tag = 'NI3E4'
        filebase = '/homecentral/srao/Documents/code/cuda/cudanw/data/pub/N3E4/'
        foldername = 'tau%s/p%s/'%(tau_syn, alpha)
    if NI == 40000 and NE == 40000:
        tag = 'NI4E4'
        filebase = '/homecentral/srao/Documents/code/cuda/cudanw/data/pub/N4E4/'
        foldername = 'tau%s/p%s/'%(tau_syn, alpha)        
    if NI == 10000 and NE == 10000:
        tag = 'NI1E4'
        filebase = '/homecentral/srao/Documents/code/cuda/cudanw/data/pub/N1E4/'
        foldername = 'tau%s/p%s/'%(tau_syn, alpha)                       
    maxLag = 2000
    ...
    filebase = '/homecentral/srao/Documents/code/cuda/cudanw/data/pub/bidir/i2i/'
    foldername = 'p%s/K%s/'%(alpha, int(K))                           
    ...
    for kTrial in range(nTrials):
        ...
        if K == 500 and NE == 20000:
            stfilename = 'spkTimes_xi0.8_theta0_0.%s_%s.0_cntrst100.0_%s_tr%s.csv'%(int(alpha), int(tau_syn), int(simDuration), 0)
            stfilename_alternate = 'spkTimes_xi0.8_theta0_0.%s0_%s.0_cntrst100.0_%s_tr%s.csv'%(int(alpha), int(tau_syn), int(simDuration), 0)
            # if int(alpha) == 9 and int(tau_syn) == 48:
            #     simDuration = 502000
            #     stfilename = 'spkTimes_xi0.8_theta0_0.%s_%s.0_cntrst100.0_%s_tr%s.csv'%(int(alpha), int(tau_syn), int(502000), 1)
            #     stfilename_alternate = 'spkTimes_xi0.8_theta0_0.%s0_%s.0_cntrst100.0_%s_tr%s.csv'%(int(alpha), int(tau_syn), int(502000), 1)
...
#    acMat = acMat / nTrials
    saveFolder = '/homecentral/srao/Documents/code/tmp/del/auto2/onepop/'
